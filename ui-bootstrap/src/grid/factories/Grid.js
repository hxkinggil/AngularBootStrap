...
.factory('Grid', ['$log', '$q', '$compile', '$parse', 'gridConstants', 'GridOptions', 'GridColumn', 'GridRow', 'GridApi', 'rowSorter', 'rowSearcher', 'GridRenderContainer',
    function($log, $q, $compile, $parse, gridUtil, gridConstants, GridOptions, GridColumn, GridRow, GridApi, rowSorter, rowSearcher, GridRenderContainer) {
 ...
 */
  Grid.prototype.preCompileCellTemplates = function() {
        ...
        this.columns.forEach(function (col) {
          var html = col.cellTemplate.replace(gridConstants.COL_FIELD, 'getCellValue(row, col)');
  ...
  Grid.prototype.getColumnSorting = function getColumnSorting() {
    ...
    self.columns.sort(rowSorter.prioritySort).forEach(function (col) {
      if (col.sort && typeof(col.sort.direction) !== 'undefined' && col.sort.direction && (col.sort.direction === gridConstants.ASC || col.sort.direction === gridConstants.DESC)) {
        sortedCols.push(col);
      }
    });
   ...
   * @param {gridConstants.ASC|gridConstants.DESC} [direction] Direction to sort by, either descending or ascending.
  ...
  Grid.prototype.sortColumn = function sortColumn(column, directionOrAdd, add) {
    ...
    if (!direction) {
      ...
      if (column.sort.direction && column.sort.direction === gridConstants.ASC) {
        column.sort.direction = gridConstants.DESC;
      }
      else if (column.sort.direction && column.sort.direction === gridConstants.DESC) {
        column.sort.direction = null;
      }
      else {
        column.sort.direction = gridConstants.ASC;
      }
    }
    else {
